...
export default function DotDuoScreen() {
  ...
  if (!profile || !profile.partner_id) {
    return (
      <ScrollView style={{ flex: 1, backgroundColor: DotFuelColors.black }} contentContainerStyle={{ padding: Spacing['2xl'], paddingBottom: 100 }}>
          ...
          {profile?.is_looking ? (
            ...
            <View style={{ gap: 24 }}>
              ...
              <Pressable
                ...
                disabled={matching}
                style={({ pressed }) => ({
                  backgroundColor: DotFuelColors.lime, borderRadius: Radius.xl,
                  padding: 16, alignItems: 'center', marginTop: 12,
                  opacity: matching ? 0.5 : pressed ? 0.9 : 1
                })}
              >
                <Text style={{ fontFamily: 'Inter', fontSize: 15, fontWeight: '800', color: DotFuelColors.black, textTransform: 'uppercase' }}>
                  {matching ? 'Matching...' : 'MATCH ME 🤝'}
                </Text>
              </Pressable>
            </View>
          )}
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: DotFuelColors.black }} behavior={process.env.EXPO_OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={90}>
      <View style={{ 
